...
(
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，自动记录当前时间' -- 创建时间
);

-- 创建categories表存储文章分类信息
CREATE TABLE categories
(
    category_id INT PRIMARY KEY AUTO_INCREMENT,                -- 分类ID，主键，自增
...
(
    post_id     INT PRIMARY KEY AUTO_INCREMENT,                                                              -- 文章ID，主键，自增
    ...
    created_at  TIMESTAMP                     DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，自动记录当前时间', -- 创建时间
    updated_at  TIMESTAMP                     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间, 自动记录当前时间',
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE,                                      -- 外键，关联到users表的user_id字段，级联删除
    FOREIGN KEY (category_id) REFERENCES categories (category_id)                                            -- 外键，关联到categories表的category_id字段，级联设置为空
);
...
(
    comment_id INT PRIMARY KEY AUTO_INCREMENT,                                          -- 评论ID，主键，自增
    user_id    INT  NOT NULL COMMENT '评论者ID，不为空',                                 -- 评论者ID
    post_id    INT  NOT NULL COMMENT '文章ID，不为空',                                   -- 文章ID
    content    TEXT NOT NULL COMMENT '评论内容，不为空',                                 -- 评论内容
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间，自动记录当前时间', -- 创建时间
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE,                 -- 外键，关联到users表的user_id字段，级联删除
    FOREIGN KEY (post_id) REFERENCES articles (post_id) ON DELETE CASCADE               -- 外键，关联到posts表的post_id字段，级联删除
);
